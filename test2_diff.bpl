// Dafny 2.3.0.10506
// Command Line Options: /print:test2.bpl test2.dfy
...
procedure CheckWellformed$$_module.__default.MultipleReturns(n#0: int) returns (j#0: int);
  free requires 0 == $FunctionContextHeight;
  modifies $Heap, $Tick;



procedure Call$$_module.__default.MultipleReturns(n#0: int) returns (j#0: int);
  // user-defined preconditions
  requires n#0 > 0;
  modifies $Heap, $Tick;
  // user-defined postconditions
  free ensures true;
  ensures j#0 >= LitInt(0);
  // frame condition
...
procedure Impl$$_module.__default.MultipleReturns(n#0: int) returns (j#0: int, $_reverifyPost: bool);
  free requires 0 == $FunctionContextHeight;
  // user-defined preconditions
  requires n#0 > 0;
  modifies $Heap, $Tick;
...
implementation Impl$$_module.__default.MultipleReturns(n#0: int) returns (j#0: int, $_reverifyPost: bool)
{
  ...
  var $decr$loop#00: int;
    ...
    // AddMethodImpl: MultipleReturns, Impl$$_module.__default.MultipleReturns
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    assume {:captureState "test2.dfy(5,0): initial state"} true;
    $_reverifyPost := false;
    // ----- assignment statement ----- test2.dfy(6,9)
    assume true;
    assume true;
    i#0 := LitInt(0);
    assume {:captureState "test2.dfy(6,12)"} true;
    // ----- while statement ----- test2.dfy(7,3)
    ...
    {
        assume {:captureState "test2.dfy(7,2): after some loop iterations"} true;
        if (!$w$loop#0)
        {
            assume true;
            assume LitInt(0) <= i#0;
        ...
        // ----- assignment statement ----- test2.dfy(11,7)
        assume true;
        assume true;
        i#0 := i#0 + 1;
        assume {:captureState "test2.dfy(11,12)"} true;
        // ----- loop termination check ----- test2.dfy(7,3)
        assert 0 <= $decr$loop#00 || n#0 - i#0 == $decr$loop#00;
        assert n#0 - i#0 < $decr$loop#00;
        assume true;
    }

    // ----- return statement ----- test2.dfy(13,3)
    // ----- assignment statement ----- test2.dfy(13,3)
    assume true;
    assume true;
    j#0 := i#0;
    assume {:captureState "test2.dfy(13,10)"} true;
    return;
}
